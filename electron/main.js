...
ipcMain.handle('chat-with-agent', async (_, message, context) => {
  ...
  try {
    ...
    async function getPageState() {
      const state = { hasPage: false, url: null, title: null, elements: [] }
      ...
      try {
        ...
        if (state.hasPage) {
          const domResult = await extractDomForAI()
          if (domResult.success) {
            state.elements = domResult.elements.filter(e => e.category !== 'other').slice(0, 50)
          }
        }
    ...
    async function executeDecision(decision, pageState) {
      ...
      switch (decision.action) {
        ...
        case 'click': {
          ...
          const elName = targetEl ? (targetEl.text || targetEl.label || decision.elementId) : decision.elementId
          ...
          await new Promise(r => setTimeout(r, 1000)) // Wait for page reaction

          if (clickResult.success) {
            result = { success: true, message: `Clicked "${elName}"`, element: elName }
          } else {
        ...
        case 'type': {
          ...
          const targetEl = pageState.elements.find(e => e.id === decision.elementId)
          const elName = targetEl ? (targetEl.label || targetEl.placeholder || decision.elementId) : decision.elementId
...
{ "action": "navigate", "url": "http://...", "reason": "why navigating here" }
{ "action": "click", "elementId": "testai-X", "reason": "why clicking this element" }
{ "action": "type", "elementId": "testai-X", "value": "text to type", "reason": "why typing this" }
{ "action": "press_enter", "reason": "to submit form or trigger action" }
{ "action": "scroll", "direction": "down|up", "reason": "why scrolling" }
{ "action": "wait", "duration": 1000, "reason": "what we're waiting for" }
{ "action": "generate_tests", "reason": "to create comprehensive test suite for this page" }
{ "action": "task_complete", "summary": "detailed description of what was accomplished" }
{ "action": "need_input", "question": "specific question for user (e.g., credentials, preferences)" }
{ "action": "cannot_proceed", "reason": "specific reason why task cannot be completed" }

REASONING PROCESS:
1. UNDERSTAND: What is the user trying to accomplish?
2. OBSERVE: What is the current page state? What elements are available?
3. PLAN: What's the most logical next step toward the goal?
4. EXECUTE: Choose ONE action that moves us closer to the goal
5. VERIFY: After action, check if we need to continue or if task is done

SMART AGENT RULES:
1. Work step by step - one action at a time
2. Always explain your reasoning in the "reason" field
3. For login/signup: Ask for credentials if not provided, don't guess
4. For forms: Fill fields in logical order (email → password → submit)
5. For navigation: Use http:// for localhost, https:// for external sites
6. For testing: Use "generate_tests" to create a comprehensive test suite
7. Be patient: Wait after clicks/navigations for page to update
8. Be observant: Check if elements exist before trying to interact
9. Be honest: Say "cannot_proceed" if something is genuinely blocked
10. Be thorough: Don't mark "task_complete" until the goal is actually achieved
